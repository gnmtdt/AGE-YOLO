...
__all__ = ['DyHeadBlock', 'DyHeadBlockWithDCNV3', 'Fusion', 'C3k2_Faster', 'C3k2_ODConv', 'Partial_conv3', 'C3k2_Faster_EMA', 'C3k2_DBB',
           ...
           'C3k2_RFAConv', 'C3k2_RFCBAMConv', 'C3k2_RFCAConv', 'Ghost_HGBlock', 'Rep_HGBlock', 'Rep_HGBlock_ECA',
           ...
           'C3k2_VSS', 'C3k2_LVMB', 'RepNCSPELAN4', 'DBBNCSPELAN4', 'OREPANCSPELAN4', 'LightFusionNet', 'DRBNCSPELAN4', 'ADown', 'V7DownSampling', 'CBLinear', 'CBFuse', 'Silence',
           ...
           'C3k2_MBRConv3', 'C3k2_VSSD', 'C3k2_TVIM', 'DPCF', 'C3k2_CSI', 'C3k2_SHSA_EPGO', 'C3k2_SHSA_EPGO_CGLU', 'C3k2_ConvAttn', 'C3k2_UniConvBlock', 'C3k2_LGLB', 'C3k2_ConverseB', 'C3k2_Converse', 'C3k2_GCConv', 'GCConv', 'MANet_GCConv', 'C3k2_CFBlock'
           ]
...
#原创
# ---------- ECA: Efficient Channel Attention (轻量注意力) ----------
class ECA(nn.Module):
    """
    Efficient Channel Attention.
    Reference idea: ECA-Net (2020). Lightweight, no FC layers.
    Implementation: global avg pool -> 1D conv (kernel k, padding (k-1)//2) -> sigmoid -> scale
    """
    def __init__(self, channels, k_size=None):
        super(ECA, self).__init__()
        self.channels = channels
        if k_size is None:
            # default adaptive kernel size from ECA paper (approx)
        ...
        # Note: using Conv1d on channel dimension; input shape (B, C, 1) -> after conv (B, C, 1)
...
# ---------- Wrapper Block: Rep_HGBlock + ECA ----------
class Rep_HGBlock_ECA(nn.Module):
    """
    Wrap original Rep_HGBlock by applying ECA after the original block output.
    Keeps the original behavior if Rep_HGBlock returns y (+ x) as before.
    """
    def __init__(self, *rep_args, eca_k=None, **rep_kwargs):
        """
        rep_args/rep_kwargs: passed to original Rep_HGBlock constructor.
        eca_k: optional kernel size for ECA (int). If None auto-compute.
        """
        super(Rep_HGBlock_ECA, self).__init__()
        # assume Rep_HGBlock is defined/imported in the same module
        self.rep_block = Rep_HGBlock(*rep_args, **rep_kwargs)
        # Determine output channels for ECA. We try to infer:
        # If Rep_HGBlock exposes an attribute for output channels (not guaranteed),
        # fallback: use rep_args[-1] or rep_args[1] depending on original signature.
        # In your earlier definition: Rep_HGBlock(c1, cm, c2, k=3, ...)
        try:
            out_ch = rep_args[2] if len(rep_args) >= 3 else rep_kwargs.get('c2', None)
        except:
            out_ch = None
        # If unknown, set a conservative default and adjust later if needed.
        if out_ch is None:
            # We'll create ECA lazily at first forward (safe fallback).
            self.eca = None
            self._lazy_eca_channels = True
            self._eca_k = eca_k
        else:
            self.eca = ECA(out_ch, k_size=eca_k)
            self._lazy_eca_channels = False

    def forward(self, x):
        y = self.rep_block(x)   # Rep_HGBlock's forward: returns y (or y + x)
        # lazy init ECA if needed
        if getattr(self, '_lazy_eca_channels', False):
            # infer channel from y
            out_ch = y.shape[1]
            self.eca = ECA(out_ch, k_size=self._eca_k)
            self._lazy_eca_channels = False
            # move to same device/dtype
            self.eca.to(y.device)
...
class DynamicFusionGate(nn.Module):
    ...
    def forward(self, x):
        attention_map = torch.sigmoid(self.attention(x))  # 动态生成特征融合权重
        return x * attention_map  # 加权输出


class LightFusionNet(nn.Module):  # 重命名模块为 LightFusionNet
    def __init__(self, c1, c2, c3, c4, c5=1):
        ...
        # 动态融合门 (DFG)，动态加权特征融合
        self.dfg = DynamicFusionGate(c3 + 2 * c4)

        # 输出通道
        self.cv4 = Conv(c3 + (2 * c4), c2, 1, 1)

    def forward(self, x):
        y = list(self.cv1(x).chunk(2, 1))  # 将输入拆分
        y.extend((m(y[-1])) for m in [self.cv2, self.cv3])  # 通过不同卷积提取特征

        # 通过动态融合门 (DFG) 对特征进行加权融合
        y = self.dfg(torch.cat(y, 1))  # 融合特征并调整权重

        return self.cv4(y)  # 输出最终特征
...
class Faster_Block_CGLU(nn.Module):
    def __init__(self,
        ...
        # CGLU替换成普通Bottleneck，消融1
        self.mlp = ConvolutionalGLU(dim)
        # self.mlp = Bottleneck(inc, dim, shortcut=True, g=1, e=0.5)

        # PConv去掉，消融2
        # self.spatial_mixing = nn.Identity()
        self.spatial_mixing = Partial_conv3(
            dim,
            n_div,
            pconv_fw_type
...
#     def __init__(self, inc) -> None:
#         """
#         inc: [in_channels_x0, in_channels_x1]
#         """
#         super().__init__()
#         # 如果两个输入通道不一致，用 1x1 卷积对齐
#         self.adjust_conv = nn.Identity()
#         if inc[0] != inc[1]:
#             self.adjust_conv = Conv(inc[0], inc[1], k=1)
#
#         # 轻量化空间注意力 (减少参数)
#         self.spatial_attn = nn.Sequential(
#             nn.Conv2d(inc[1] * 2, 1, kernel_size=3, padding=1),  # 7x7 -> 3x3
#             nn.Sigmoid()
#         )
#
#         # 融合卷积 (1x1)
...
#         # 通道切分
#         c = x0.size(1)  # 输出通道数
#         x0_weight, x1_weight = torch.split(x_concat, [c, c], dim=1)
#
#         # 融合：互相增强
#         x0_out = x0 + x1_weight * self.cross_weight_x0
#         x1_out = x1 + x0_weight * self.cross_weight_x1
