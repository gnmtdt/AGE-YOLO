...
#
#         # 修改：非共享卷积 + 标准卷积（非深度可分离）
#         self.share_conv = nn.ModuleList([
#             nn.Sequential(
#                 Conv_GN(hidc, hidc, 3),  # 改为标准卷积，去掉g=hidc
#                 Conv_GN(hidc, hidc, 1)
...
#         # 非共享分类和回归分支
#         self.cv2 = nn.ModuleList([
#             nn.Conv2d(hidc, 4 * self.reg_max, 1) for _ in range(self.nl)
#         ])
#         self.cv3 = nn.ModuleList([
#             nn.Conv2d(hidc, self.nc, 1) for _ in range(self.nl)
#         ])
#
...
#         for i in range(self.nl):
#             x[i] = self.conv[i](x[i])
#             x[i] = self.share_conv[i](x[i])  # 非共享卷积
#             x[i] = torch.cat((self.scale[i](self.cv2[i](x[i])), self.cv3[i](x[i])), 1)
#         # ... 其余代码保持不变
#
#     def bias_init(self):
#         """Initialize Detect() biases, WARNING: requires stride availability."""
...
#             m.cv3[i].bias.data[: m.nc] = math.log(5 / m.nc / (640 / 16) ** 2)  # cls
...
class Detect_LSCD(nn.Module):
    ...
    def __init__(self, nc=80, hidc=256, ch=()):
        ...
        #     nn.Conv2d(x, hidc, 3, padding=1, bias=False),
        #     nn.BatchNorm2d(hidc),
        ...
        #     nn.Conv2d(hidc, hidc, 3, padding=1, groups=hidc, bias=False),
        #     nn.BatchNorm2d(hidc),
        #     nn.SiLU(inplace=True),
        #     nn.Conv2d(hidc, hidc, 1, bias=False),
        #     nn.BatchNorm2d(hidc),
        #     nn.SiLU(inplace=True)
        # )
