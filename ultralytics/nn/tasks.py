...
DETECT_CLASS = (Detect, Detect_DyHead, Detect_AFPN_P2345, Detect_AFPN_P2345_Custom, Detect_AFPN_P345, Detect_AFPN_P345_Custom, Detect_Efficient, DetectAux, Detect_SEAM, Detect_MultiSEAM, 
                Detect_DyHeadWithDCNV3, Detect_DyHeadWithDCNV4, Detect_DyHead_Prune, Detect_LSCD, SIFHead, Detect_TADDH, Detect_LADH, Detect_LSCSBD, Detect_LSDECD, Detect_RSCD, Detect_FGIF, Detect_LQE,
                Detect_LSCD_LQE)
...
C3K2_CLASS = (C3k2_Faster, C3k2_ODConv, C3k2_Faster_EMA, C3k2_DBB, C3k2_DeepDBB, C3k2_WDBB, C3k2_CloAtt, C3k2_SCConv, C3k2_ScConv, C3k2_EMSC, C3k2_EMSCP, C3k2_KW, C3k2_DySnakeConv, C3k2_DCNv2, C3k2_DCNv3,
              ...
              C3k2_ConverseB, C3k2_Converse, C3k2_GCConv, C3k2_CFBlock
              )
...
C2PSA_CLASS = (C2PSA, C2BRA, C2CGA, C2DA, C2DPB, C2Pola, C2TSSA, C2ASSA, C2PSA_DYT, C2TSSA_DYT, C2Pola_DYT, C2PSA_FMFFN, C2PSA_CGLU, C2PSA_SEFN, C2PSA_Mona, C2PSA_SEFFN, C2TSSA_DYT_Mona, C2TSSA_DYT_Mona_SEFN, C2TSSA_DYT_Mona_SEFFN, C2PSA_EDFFN,
               C2TSSA_DYT_Mona_EDFFN, C2MSLA, C2PSA_EPGO)
...
def parse_model(d, ch, verbose=True, warehouse_manager=None):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in ((
            ...
            SPPF_LSKA, CSP_EDLAN, nn.Conv2d, HWD, RepNCSPELAN4, DBBNCSPELAN4, OREPANCSPELAN4, LightFusionNet, DRBNCSPELAN4, RepNCSPELAN4_CAA, V7DownSampling,
            ...
            LDConv, CSP_MSCB, CSP_PMSFA, RFAConv, RFCBAMConv, RFCAConv, CSP_FreqSpatial, MANet, MANet_FasterBlock, MANet_FasterCGLU, FC_MANet,
            ...
            if m in (DySnakeConv,):
                c2 = c2 * 3
            if m in (RepNCSPELAN4, DBBNCSPELAN4, OREPANCSPELAN4, LightFusionNet, DRBNCSPELAN4, RepNCSPELAN4_CAA):
            ...
            if m in ((
                ...
                MANet_FasterBlock, MANet_FasterCGLU, FC_MANet, MANet_Star, CSP_MSCB_SC, MANet_GCConv
            ) + C3K2_CLASS + A2C2F_CLASS + C2PSA_CLASS):
            ...
            args = [ch[f], *args]
            c2 = args[0]
        elif m in (HGStem, HGBlock, Ghost_HGBlock, Rep_HGBlock, Rep_HGBlock_ECA, Dynamic_HGBlock, EIEStem):
            c1, cm, c2 = ch[f], args[0], args[1]
            if c2 != nc:  # if c2 not equal to number of classes (i.e. for Classify() output)
                c2 = make_divisible(min(c2, max_channels) * width, 8)
                cm = make_divisible(min(cm, max_channels) * width, 8)
            args = [c1, cm, c2, *args[2:]]
            if m in (HGBlock, Ghost_HGBlock, Rep_HGBlock, Rep_HGBlock_ECA, Dynamic_HGBlock):
            ...
            if m in SEGMENT_CLASS:
                ...
                if m in (Segment_LSCD, Segment_TADDH, Segment_LSCSBD, Segment_LSDECD, Segment_RSCD, Segment_LSCD_LQE):
                    args[3] = make_divisible(min(args[3], max_channels) * width, 8)
            if m in (Detect_LSCD, Detect_TADDH, SIFHead, Detect_LSCSBD, Detect_LSDECD, Detect_RSCD, Detect_LSCD_LQE):
        ...
        # --------------ASF--------------
            ...
            c2 = ch[f[1]]
            args = [c2, *args]
        elif m in {ACF_Module, ContextGuideFusionModule}:
            c1 = [ch[x] for x in f]
            c2 = 2 * c1[1]
            args = [c1]
        # elif m in {PSA}:
